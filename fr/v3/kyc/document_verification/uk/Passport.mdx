...
description: "Extrayez les détails d'identité d'un passeport britannique à l'aide de l'OCR."
openapi: "POST /api/onboarding/document_verification/uk/passport"
---

Vérifiez le passeport britannique d'un client en téléversant une photo de la page de données — aucune saisie manuelle du numéro de passeport requise. Extraction OCR plus une concordance faciale requise par rapport à un selfie — pas de vérification de vivacité (pas de vidéo, pas de défi de clignement/mouvement).
...
<Note>
  La vérification de documents au Royaume-Uni prend actuellement en charge uniquement les passeports — le national ID, le permis de conduire et les autres types de documents britanniques ne sont pas encore disponibles.
</Note>
...
| `data.extra_fields` | object | Autres champs lus par l'OCR sur la page de données — `place_of_birth`, `issuing_authority`, et `mrz_line1`/`mrz_line2` si la zone lisible par machine était lisible, exactement tels qu'imprimés |
