...
```kotlin
dependencies {
    implementation("io.github.386konsult:android-sdk:1.0.6")
}
```
...
<Note>
  **1.0.7 is the release to move to when it lands.** It carries [Branding](#branding),
  a `RESULT_STATUS` that finally holds the real verdict, and a fix for a blank
  liveness camera after skipping the back-side capture. Anything on this page marked
  "Requires 1.0.7" is not in 1.0.6.
</Note>
