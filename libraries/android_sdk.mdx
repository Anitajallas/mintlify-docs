...
```kotlin
dependencies {
    implementation("io.github.386konsult:android-sdk:1.2.0")
}
```
...
composed photograph: 720px on the long edge at JPEG quality 72. From 1.2.0, in document
mode the autoshot goes up with the video, because the entry already exists from the
document check. In data mode it goes up with `liveness.create` as before.
