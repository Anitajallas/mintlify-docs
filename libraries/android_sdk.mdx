...
```kotlin
dependencies {
    implementation("io.github.386konsult:android-sdk:1.2.1")
}
```
...
<Note>
  **Take 1.2.1 or later, not 1.2.0.** Up to 1.2.0, on some phones the autoshot and
  snapshot upload as a solid green image with vertical stripes instead of the user's
  face. It depends on how the phone's camera lays out its frames, so it shows on some
  models and never on others. Liveness and the face match are unaffected, because
  both read the frame correctly; only the stills on the dashboard are wrong. 1.2.1 is
  that one fix and changes nothing else.
</Note>

<Note>
  **1.2.0 checks the ID before the selfie, and holds the selfie until the face is
  in the oval.** The ID now goes up on its own and the backend reads it and checks
  it against the government registry before the liveness camera opens, so a
  cropped or misread ID is turned back with a retake screen instead of failing
  after the whole flow. The selfie and the scan then wait until the user's whole
  face is inside the oval, with no timeout. Two new ways to end the flow return
  `TYPE_FAILURE`. Read [Document check](#document-check) and [Liveness](#liveness)
  before you upgrade.
</Note>
...
<Note>
  **1.1.2 changes when the liveness selfie is taken.** Up to 1.1.1 it fired on a
  ...
  get into frame, and nothing at all for one who is already there. 1.2.0 removes
  that 2.5 second limit; see [Liveness](#liveness).
</Note>
...
| `RESULT_ERROR_MSG` | `String` | `TYPE_FAILURE` only: why the flow ended without a verification, e.g. `"SmartComply: apiKey is required"`, or from 1.2.0 the backend's reason for rejecting the ID |
...
<Note>
  ...
  other extras. Before 1.0.7 the same input crashed the host app.

  From **1.2.0** `TYPE_FAILURE` is also returned when the user taps **End session** on a rejected
  ID, with the backend's reason in `RESULT_ERROR_MSG`, and when they tap **Close** after the session
  has used its three document attempts, with `"Verification sent to manual review"`. See
...
| Welcome | Brand splash, country picker, and ID type selection. From 1.2.0 **Continue** stays disabled until a country is picked when more than one is configured |
...
| Document Capture (back) | Shown when the channel's `requires_back_side` is `required` or `optional`. The server decides, not the app. On `optional` the screen also shows a **This ID has no back side** control, which skips straight to the document check |
| Document check | From 1.2.0. "Reading your document" while the backend reads the ID and checks it against the registry. See [Document check](#document-check) |
| Confirm details | The name, ID number, expiry and photo read from the ID. **Yes, continue** opens liveness; **No, retake my ID** goes back to the camera |
| Document rejected | The backend's reason, with a retake or a different ID, and **End session** |
...
From **1.2.0** the ID is checked before the liveness camera opens. Up to 1.1.2 it went up
...
not billed. **End session** and **Close** return `TYPE_FAILURE` with the reason in
`RESULT_ERROR_MSG`.
...
Before the review screen the SDK checks the **front** photo contains a face. A failure is a **warning, not a block**: the user can still confirm, because a false negative would otherwise trap someone holding a genuine but unusual ID. The backend is the check that actually rejects, and from 1.2.0 it does so before the selfie: see [Document check](#document-check). The back photo is not checked on the device.
...
Two stills leave the device. The **autoshot** is taken before the scan starts and the
**snapshot** once both actions are done. Both come off the analysis stream, so neither is a
composed photograph: 720px on the long edge at JPEG quality 72. Up to 1.2.0 some phones
uploaded both as a green, striped image; see the 1.2.1 note under Add the dependency.
From 1.2.0, in document mode the autoshot goes up with the video, because the entry
already exists from the document check. In data mode it goes up with `liveness.create` as before.

**From 1.2.0 nothing starts until the whole face is inside the oval.** After a 400ms settle
the SDK holds the autoshot until three consecutive frames show the face inside the oval the
user is looking at, and there is no timeout. Up to 1.1.2 it gave up after 2.5 seconds and
took whatever frame it had, which is how a selfie of nobody could still be uploaded, and up
to 1.1.1 it fired on a fixed timer without looking for a face at all.

- **The prompt says what is wrong.** "Position your face in the oval" when there is no face,
  "Move back a little" when the face is centred but too big to fit, and "Fit your whole face
  inside the oval" when it is off to one side or too low. It stays up until the face is in.
- **During the scan an action only counts while the face is in the oval.** If the face is out
  for more than about half a second the prompt comes back and nothing is credited until it
  returns. The grace keeps a head turn, which swings the face across the edge, from flashing
  the prompt.
- **The 20 second cap starts after the autoshot**, so a user who takes a while to get into
  position still has the full window.
- **How the oval is judged.** ML Kit's face box is mapped onto the preview as it is displayed,
  and the face is taken as 80% of the box because the box runs wider than the face: on a
  TECNO KI7, a face comfortably inside the oval measured a box about 1.1 times its width. The
  face must also be wholly inside the camera frame with an 8% margin, at least 18% of the frame
  width, and within 30 degrees of facing the camera.
...
| `TYPE_FAILURE` after a rejected ID | From 1.2.0: the user tapped **End session** on a rejected ID, or **Close** after three failed document checks | Read `RESULT_ERROR_MSG`. The entry is failed or in manual review, and nothing was billed |
