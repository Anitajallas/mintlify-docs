...
<Note>
  **1.0.7 changes two behaviours.** `RESULT_STATUS` now holds the real verdict, and
  the success screen no longer means a pass. Read [Result Extras](#result-extras)
  before upgrading from 1.0.6 or below. It also adds [Branding](#branding) and a
  custom backend host, and fixes a blank liveness camera after skipping the
  back-side capture.
</Note>
...
Document photos are capped at **5 MB** server-side, in JPG or PNG. The two routes are treated differently: an in-SDK camera capture is cropped to the guide box and re-encoded at JPEG quality 85, while an **Upload from gallery** pick is downscaled to 1600px on the long edge and re-encoded, so a full-resolution phone photo no longer fails with `File too large. Maximum size is 5MB.`
