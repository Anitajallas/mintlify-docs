...
<Steps>
  <Step title="createSession()">
    Authenticates your app using `apiKey` + `clientId`.

    **Endpoint:** `POST /v1/session/create` → returns a **session token** used by all subsequent calls.
  </Step>
  <Step title="onboarding.verify()">
    Verifies the user's identity against a government ID (BVN or NIN).

    **Endpoint:** `POST /v1/onboarding/verify` → returns **Identity confirmed**.
  </Step>
  <Step title="liveness.startCheck()">
    ...
    - **Autoshot captured** → `POST /v1/liveness/create` → returns an Entry ID.
    - **Video uploaded** → `POST /v1/liveness/submit` → returns the result.
  </Step>
</Steps>
