...
4. Select **Up to Next Major Version** and enter `1.0.3`
...
<Note>
  **1.0.3 changes two behaviours.** `FlowResult.status` now holds the real verdict, and the success
  ...
  iOS and Android are versioned separately. iOS 1.0.3 and Android 1.0.7 carry the same feature set;
  the numbers differ because the two have always been on their own tracks.
</Note>
...
Before the review screen the SDK checks the **front** photo shows a face and legible text: it
needs a face and at least three text regions. A failure is a **warning, not a block**. The user
can still confirm, because a false negative would otherwise trap someone holding a genuine but
unusual ID. The backend OCR is the check that actually rejects. The back photo is not checked.
