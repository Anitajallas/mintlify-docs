...
- **Liveness-Only Mode** — Set your SDK Config's `verification_type` to `liveness_only` to skip ID/document collection entirely and only confirm the customer is a live person. The widget goes straight from the welcome screen into the camera scan — no country picker, no ID-type picker, no channels to configure.
...
For **liveness-only** SDK Configs — `verification_type: "liveness_only"` — there's no ID number and no document, so `customer_profile`/`document` are both absent. `biometrics` carries the real liveness verdict instead of an unconditional pass:
...
<Note>
  `face_match` and `face_analysis` are always `null` for `liveness_only` — there's nothing to compare the selfie against, only a direct liveness/anti-spoofing check on the recorded video, so neither a face-match result nor the gender/emotion/quality breakdown the other two verification types populate applies here. `biometrics.liveness_verified` mirrors `status === "passed"`; `biometrics.liveness_passed` is the underlying scored verdict the check actually ran, which is what determines that status in the first place.
</Note>
