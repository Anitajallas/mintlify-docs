...
Use `identifier` (the ID value) and `identifier_type` (the key, e.g. `bvn`, `national_id`, `ghana_card`) for the applicant's identification — this works for Nigeria and every other supported country.
