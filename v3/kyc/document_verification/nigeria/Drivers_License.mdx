...
<Note>
  **Extra Fields** — `extra_fields` carries every other field the license actually printed that doesn't have its own dedicated field above. Nigerian licenses print more of these than other countries' — expect anywhere from a few fields to the full set shown here, and expect `null`/absent fields to be dropped rather than sent as `null`, since a field that wasn't printed on the document isn't guessed at.
</Note>
