...
<Warning>
  `registration_number` must include the CAC type prefix (e.g. `RC9435541`, not `9435541`) — the endpoint does not infer the prefix from a separately-sent company type. RC-prefixed (limited company) numbers are confirmed working. BN-prefixed (business name) numbers currently return a provider-side error (`youverify gateway error 500`) even when correctly formatted — this looks like a gap on Youverify's side specific to BN registrations, not a formatting issue.
</Warning>
