...
| `identifier` | number | Yes | Business identifier value (e.g. RC number for Nigeria) |
| `identifier_type` | string | Yes | Key of the identifier type (e.g. `rc_number`) |
...
<Note>
`rc_number` (top-level) is a legacy field kept for backward compatibility (Nigeria only). Use `identifier` (the ID value) and `identifier_type` (the key, e.g. `rc_number`) instead — this works for Nigeria and every other supported country.
</Note>
