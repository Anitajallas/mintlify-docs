...
| `bvn` | string | No | Legacy field, Nigeria only. Superseded by `identifier` + `identifier_type` — use those instead |
| `identifier` | number | No | Customer identifier value (e.g. BVN for Nigeria, a Kenya national ID number). Use with `identifier_type` instead of `bvn` |
| `identifier_type` | string | No | Key of the identifier type (e.g. `bvn`, `national_id`, `ghana_card`). Required when `identifier` is provided |
...
<Note>
`bvn` is a legacy field kept for backward compatibility (Nigeria only). Use `identifier` (the ID value) and `identifier_type` (the key, e.g. `bvn`, `national_id`, `ghana_card`) instead — this works for Nigeria and every other supported country.
</Note>
