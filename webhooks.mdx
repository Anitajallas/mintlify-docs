...
The secret is generated for you when the SDK Config is created. Get it from the
Adhere Dashboard: go to **Settings → Integrations → SDK**, open the menu on your
config's row and choose **Integration**. The secret is shown there with your
Config ID, with a button to copy it.

Treat it like a password. Anyone holding it can forge a validly signed webhook
carrying fabricated verification results, so store it the way you store any other
server-side credential and never ship it in client code.
