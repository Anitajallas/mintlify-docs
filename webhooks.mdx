...
The secret is generated for you when the SDK Config is created and is deliberately
absent from the ordinary config read, since anyone holding it can forge a validly
signed webhook. Retrieve it explicitly instead:

```bash
curl https://adhere-api.smartcomply.com/v1/sdk-config/<config-id>/webhook-secret/ \
  -H "Authorization: Token <your-dashboard-token>"
```
...
Only the **account administrator** can call this, not an employee with the admin
role, and not the owner of a different account. Every call is logged.

To rotate the secret, `POST` to the same URL. The new value takes effect
immediately, so deploy it before your next verification completes or signatures
will fail in the gap.
